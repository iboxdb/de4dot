    ...
    public class Renamer
    {
        ...
        void RestorePropertiesFromNames2(IEnumerable<MethodNameGroup> allGroups)
        {
            ...
            foreach (var group in allGroups)
            {
                ...
                var oldfullname = TypeInfo.GetMyFullName(groupMethod.MethodDef);
                ...
                if (Utils.StartsWith(methodName, "get_", StringComparison.Ordinal))
                {
                    ...
                    foreach (var method in group.Methods)
                    {
                        if (onlyRenamableMethods && !memberInfos.Type(method.Owner).NameChecker.IsValidPropertyName(
                            oldfullname))
                            continue;
                ...
                else if (Utils.StartsWith(methodName, "set_", StringComparison.Ordinal))
                {
                    ...
                    foreach (var method in group.Methods)
                    {
                        if (onlyRenamableMethods && !memberInfos.Type(method.Owner).NameChecker.IsValidPropertyName(oldfullname))
                            continue;
            ...
            foreach (var group in groups.GetAllGroups())
            {
                ...
                foreach (var method in group.Methods)
                {
                    ...
                    for (int i = 0; i < argNames.Length; i++)
                    {
                        ...
                        var argFullName = MemberInfo.GetMyFullName(method.MethodDef) + " arg:" + argName;
                      
                        if (argName == null || !nameChecker.IsValidMethodArgName(argFullName))
                            continue;
        ...
        string[] GetValidArgNames(MethodNameGroup group)
        {
            ...
            foreach (var method in methods)
            {
                ...
                for (int i = 0; i < argNames.Length; i++)
                {
                    ...
                    var argFullName = MemberInfo.GetMyFullName(method.MethodDef) + " arg:" + argName;
                    if (nameChecker == null ||
                        nameChecker.IsValidMethodArgName(argFullName))
                        argNames[i] = argName;
        ...
        string PrepareRenameProperty(MethodNameGroup group, bool renameOverrides, out string overridePrefix)
        {
            ...
            else if (mustUseOldPropName || propDef.Owner.Module.ObfuscatedFile.NameChecker.IsValidPropertyName(propInfo.oldFullName))
        ...
        bool HasInvalidMethodName(MethodNameGroup group)
        {
            foreach (var method in group.Methods)
            {
                ...
                if (typeInfo.NameChecker.IsValidMethodName(methodInfo.oldFullName))
                    return false;
            }
            return true;
        }
